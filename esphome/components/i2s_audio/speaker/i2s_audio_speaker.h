...
class I2SAudioSpeakerBase : public I2SAudioOut, public speaker::Speaker, public Component {
 ...
 protected:
  ...
  // the snapshot; they never reach into the ring buffer or the audio source, both of which are
  // single-consumer-thread objects (see RingBufferAudioSource's contract). Publishing through one
  // seqlock also makes the sum atomic, so a reader cannot observe the ring already drained while the
  // DMA count has not yet been updated -- nor a duration from one iteration stamped with the instant
  // of the next.
  //
  // The latency reading includes the DMA descriptors' silence padding, and that is the point.
  // Lockstep write records count only REAL frames, so the played-frames callback -- and any accounting
  // built on it -- cannot see padding, even though padding still costs time to render. The buffered
  // reading excludes it, for callers auditing their own written-minus-played count.
  //
  // Staleness is bounded by one task iteration (one DMA buffer, DMA_BUFFER_DURATION_MS) and is
